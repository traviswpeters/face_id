function [SegNewJagged, segImageBoxed] = grabCut(ImName, xMin, xMax, yMin, yMax, boundingBox)
...
fp = [xMin yMin; xMax yMax]; %opposite corners of the rectangle

fp = ginput(2); %opposite corners of the rectangle
fp = round(fp);
...
im = rgb2gray(SegNewImage);
[indI, indJ]=find(im ~=0);

SegNewJagged = SegNewImage;

segImageBoxed = I(min(indI):max(indI), min(indJ):max(indJ));
...
subplot(132)
imshow(uint8(SegNewJagged));
subplot(133)
imshow(segImageBoxed)
